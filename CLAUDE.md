...
- **`genetic/`** — the GA layer on top of DEAP.
  ...
  - `main.py` — driver. Two top-level constants: `PAD` (drives machine palette + pad_config lookup) and `MIDI_CHANNEL` (where MIDI sends actually go; default `= PAD`, override when Rytm routing reassigns pad → channel).
